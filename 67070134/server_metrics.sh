...
[ -z "$iface" ] && iface=$(ip -o -4 addr show | awk 'NR==1{print $2}')
...
# 7) Top
top_raw=$(top -bn1 | head -n 20)
top_json=$(printf '%s' "$top_raw" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))')
